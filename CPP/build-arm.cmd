...
nmake %OPTS%
IF %errorlevel% NEQ 0 echo "Error x64 @ brotli-arm.dll" >> %ERRFILE%
copy arm\brotli.dll %OUTDIR%\brotli-arm.dll

cd %ROOT%\Bundles\Codec_lizard
nmake %OPTS%
IF %errorlevel% NEQ 0 echo "Error x64 @ lizard-arm.dll" >> %ERRFILE%
copy arm\lizard.dll %OUTDIR%\lizard-arm.dll

cd %ROOT%\Bundles\Codec_lz4
nmake %OPTS%
IF %errorlevel% NEQ 0 echo "Error x64 @ lz4-arm.dll" >> %ERRFILE%
copy arm\lz4.dll %OUTDIR%\lz4-arm.dll

cd %ROOT%\Bundles\Codec_lz5
nmake %OPTS%
IF %errorlevel% NEQ 0 echo "Error x64 @ lz5-arm.dll" >> %ERRFILE%
copy arm\lz5.dll %OUTDIR%\lz5-arm.dll

cd %ROOT%\Bundles\Codec_zstd
nmake %OPTS%
IF %errorlevel% NEQ 0 echo "Error x64 @ zstd-arm.dll" >> %ERRFILE%

...
static LPCTSTR kPluginsTopic = _T("FM/options.htm#plugins");
  ...
  public CComObjectRoot
{
  UString _pluginName;
public:
BEGIN_COM_MAP(CPluginOptionsCallback)
  COM_INTERFACE_ENTRY(IPluginOptionsCallback)
END_COM_MAP()
DECLARE_NOT_AGGREGATABLE(CPluginOptionsCallback)
DECLARE_NO_REGISTRY()
...
STDMETHODIMP CPluginOptionsCallback::GetProgramFolderPath(BSTR *value)
{
  *value = 0;
  ...
  if (!::GetProgramFolderPath(folder))
    return E_FAIL;
  CComBSTR valueTemp = GetUnicodeString(folder);
...
STDMETHODIMP CPluginOptionsCallback::GetProgramPath(BSTR *value)
{
  *value = 0;
  ...
  if (!::GetProgramFolderPath(folder))
    return E_FAIL;
  CComBSTR valueTemp = GetUnicodeString(folder) + GetDefaultProgramName();
  *value = valueTemp.Detach();
  return S_OK;
}
...
{
  CComBSTR valueTemp = UString(L"Software\\7-Zip\\FM\\Plugins\\") + _pluginName;
  *value = valueTemp.Detach();
  return S_OK;
}
...
void CPluginsPage::OnButtonOptions()
{
  ...
  CComPtr<IPluginOptions> pluginOptions;
  if (pluginOptions.CoCreateInstance(pluginInfo.OptionsClassID) != S_OK)
  {
    MessageBoxW(HWND(*this), L"There are no options", L"7-Zip", 0);
    return;
  }
  CComObjectNoLock<CPluginOptionsCallback> *aCallbackSpec =
    new CComObjectNoLock<CPluginOptionsCallback>;
  CComPtr<IPluginOptionsCallback> aCallback(aCallbackSpec);
  aCallbackSpec->Init(pluginInfo.Name);
  pluginOptions->PluginOptions(HWND(*this), aCallback);
}
